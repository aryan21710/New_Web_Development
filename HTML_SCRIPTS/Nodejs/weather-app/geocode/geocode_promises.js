...
var geocodeAddr=((argv)=> {
	return new Promise((resolve,reject)=> {
			...
			console.log('CALLING THE GEOCODE PROMISE ADDRESS FUNCTION NOW:-');
			...
			request({
				...
				if (error) {
					reject('ERROR OCCURED NOT ABLE TO REACH THE SERVER:-');
				} else if (body.info.messages.length > 0) {
					if (body.info.messages[0].toString().search('Illegal') != -1) {
						reject('PLEASE ENTER A VALID ADDRESS AND TRY AGAIN');
					}
				} else {
